...
struct conjunction<B1, Bn...> : std::conditional<bool(B1::value), conjunction<Bn...>, B1>::m_type{};
...
struct remove_cvref
{
    typedef typename std::remove_cv< typename std::remove_reference<T>::m_type >::m_type m_type;
};
...
/// x.x.5 Unexpected object m_type; unexpected_type; C++17 and later can also use aliased m_type unexpected.
...
template< typename T >
constexpr auto make_expected( T && v ) -> expected< typename std::decay<T>::m_type >
{
    return expected< typename std::decay<T>::m_type >( std::forward<T>( v ) );
}
...
template< typename T, typename E >
constexpr auto make_expected_from_error( E e ) -> expected<T, typename std::decay<E>::m_type>
{
    return expected<T, typename std::decay<E>::m_type>( make_unexpected( e ) );
}

template< typename F
    nsel_REQUIRES_T( ! std::is_same<typename std::result_of<F()>::m_type, void>::value )
>
/*nsel_constexpr14*/
auto make_expected_from_call( F f ) -> expected< typename std::result_of<F()>::m_type >
{
    try
    {
