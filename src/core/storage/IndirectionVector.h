...
public:
	...
	[[nodiscard]] Val get_from_slot(const SlotId n) {
		if (n >= m_slots.size())
			throw BadRead(" - trying to access slot = {} but out of bounds", n);
		...
		if (!deserializer.Read(&val))
			throw BadRead(" - deserializer fails reading val_data for slot", n);
		return val;
	}
	...
	void set_to_slot(const Val &val, const auto sz = sizeof(Val)) {
		...
		if (!serializer.Write(&val)
			throw BadWrite(" - serializer fails writing val_data for slot", n);
	...
	void replace_in_slot(const SlotId n, const Val &new_val, const auto new_val_sz = sizeof(Val)) {
		...
		auto new_val_pos = m_slot_pager.alloc_inner(sz);
		m_slot_pager.free_inner(slot.pos, slot.size);
		...
		if (!serializer.Write(&new_val)
			throw BadWrite(" - serializer fails updating with new_val for slot", n);
	...
	void save() {
		...
		if (!serializer.Write(*this)
			throw BadWrite(" - serializer fails writing indirection vector", n);
	}
	...
	void load() {
		...
		if (!deserializer.Read(this))
			throw BadRead(" - deserializer fails reading indirection vector", n);
	}
	...
	explicit IndirectionVector(std::string_view identifier, ActionOnConstruction action = ActionOnConstruction::Load, Args &&...args)
		...
		switch (action) {
			...
			break; case DoNotLoad: DO_NOTHING;
		}
		// clang-format on
	}
